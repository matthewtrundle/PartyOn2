...
export default function ExperienceSelector<T extends string>({
  modes,
  currentMode,
  onModeChange,
  modeLabels,
  ...
  return (
    <div className="fixed top-20 right-4 z-50 group">
      <div className="bg-black/20 backdrop-blur-xl border border-white/30 rounded-2xl p-2 shadow-2xl">
        <div className="flex items-center gap-1 bg-white/90 backdrop-blur-md rounded-xl p-1 shadow-lg">
          {modes.map((mode) => (
            <button
              key={mode}
              onClick={() => onModeChange(mode)}
              className={`relative px-3 py-2 rounded-lg text-xs font-semibold transition-all duration-300 ${
                currentMode === mode 
