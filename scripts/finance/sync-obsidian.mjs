#!/usr/bin/env node
/**
 * Sync the GitHub finance-mirror into the local Obsidian vault.
 *
 * Mirrors three folders:
 *   docs/finance/weekly/             → <vault>/Memory/Finance/Briefings/
 *   docs/finance/recommendations/    → <vault>/Memory/Finance/Recommendations/
 *   docs/finance/decisions/          → <vault>/Memory/Finance/Decisions/   (when present)
 *
 * Idempotent: compares the remote SHA against a local cache file
 * (Memory/Finance/.sync-state.json) and only writes when something changed.
 *
 * Usage:
 *   node scripts/finance/sync-obsidian.mjs                    # one-shot
 *   node scripts/finance/sync-obsidian.mjs --watch             # poll every 5 min
 *   node scripts/finance/sync-obsidian.mjs --dry-run           # show what would change
 *
 * Env (in .env.local):
 *   GITHUB_REPO_TOKEN            required — repo:contents:read scope
 *   OBSIDIAN_VAULT_FINANCE       optional — defaults to
 *                                 /Users/allan/Projects/Obsidian/Obsidian/PartyOn2/Memory/Finance
 *   GITHUB_REPO_OWNER            defaults to allan-cmyk
 *   GITHUB_REPO_NAME             defaults to PartyOn2
 *   GITHUB_REPO_BRANCH           defaults to main
 *
 * Same pattern as scripts/operations/sync-obsidian.mjs — copy-and-paramaterise
 * was preferred over abstraction so each director's sync stays legible on its
 * own. Pull these together if the duplication ever becomes a maintenance drag.
 */
...
const VAULT = process.env.OBSIDIAN_VAULT_FINANCE
  ?? '/Users/allan/Projects/Obsidian/Obsidian/PartyOn2/Memory/Finance';
...
const FOLDER_MAP = [
  { remote: 'docs/finance/weekly', local: path.join(VAULT, 'Briefings') },
  { remote: 'docs/finance/recommendations', local: path.join(VAULT, 'Recommendations') },
  { remote: 'docs/finance/decisions', local: path.join(VAULT, 'Decisions') },
];
