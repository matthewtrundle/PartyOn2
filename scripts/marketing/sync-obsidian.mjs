#!/usr/bin/env node
/**
 * Sync the GitHub marketing-mirror into the local Obsidian vault.
 *
 * Mirrors three folders:
 *   docs/marketing/weekly/             → <vault>/Memory/Marketing/Briefings/
 *   docs/marketing/recommendations/    → <vault>/Memory/Marketing/Recommendations/
 *   docs/marketing/decisions/          → <vault>/Memory/Marketing/Decisions/   (when present)
 *
 * Idempotent: compares the remote SHA against a local cache file
 * (Memory/Marketing/.sync-state.json) and only writes when something changed.
 *
 * Usage:
 *   node scripts/marketing/sync-obsidian.mjs                    # one-shot
 *   node scripts/marketing/sync-obsidian.mjs --watch             # poll every 5 min
 *   node scripts/marketing/sync-obsidian.mjs --dry-run           # show what would change
 *
 * Env (in .env.local):
 *   GITHUB_REPO_TOKEN           required — repo:contents:read scope
 *   OBSIDIAN_VAULT_MARKETING    optional — defaults to /Users/allan/Projects/Obsidian/Obsidian/PartyOn2/Memory/Marketing
 *   GITHUB_REPO_OWNER           defaults to allan-cmyk
 *   GITHUB_REPO_NAME            defaults to PartyOn2
 *   GITHUB_REPO_BRANCH          defaults to main
 *
 * Cowork handoff:
 *   Run before starting a marketing session in Cowork. The vault will reflect every
 *   triage-queue status change (and any new weekly briefing) so Cowork can read the
 *   current state.
 */
...
// ---- env loading (basic, mirrors how scripts/ops/* loads .env.local) ----
...
// Auth: prefer GITHUB_REPO_TOKEN env var, fall back to `gh auth token` (the user's
// stored GitHub CLI auth). The script only does READS, so any token with repo:contents
// scope works.
...
const VAULT = process.env.OBSIDIAN_VAULT_MARKETING
  ?? '/Users/allan/Projects/Obsidian/Obsidian/PartyOn2/Memory/Marketing';
...
function reportSummary(s) {
  ...
  for (const w of s.written) {
    lines.push(`  ${w.action === 'create' ? '+' : '~'} ${w.local}`);
  }
  for (const e of s.errors) {
    lines.push(`  ! ${e.key ?? e.folder}: ${e.error}`);
  }
